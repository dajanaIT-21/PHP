...
echo "<br> Elo! das ist an girls names";
#tüdrukute nimed
$nimed = array('mart', 'kati', 'juhan', 'miku', 'uku', 'kotsa', 'jassa', 'mario');
sort($nimed);
...
#tüdrukute nimed kaks
$names = array('mart', 'kati', 'juhan', 'miku', 'uku', 'kotsa', 'jassa', 'mario');

$kolmnime = array_slice($names, 0, 3);

foreach ($kolmnime as $name) {
  echo $name . "\n";
}
echo end($names);

#suvakas nimi
echo "<br> taiesti suvakas nimi = ";
$random = rand(0, count($names)-1);
echo $names[$random];

#autod
echo "<br><br> Autode arv: ";
...
$nubrid = array("1GKS1GKC8FR966658", "1FTEW1C87AK375821", "1G4GF5E30DF760067", "1FTEW1CW9AF114701", "WAUGGAFC8CN433989", "3G5DA03E83S704506", "4JGDA2EB0DA207570", 
...
$a_arvud = count($autod);
$teisepask = count($nubrid);


echo $a_arvud;

if($a_arvud == $teisepask){
  echo "<br>masiivid on võrdsed :)";

}

$toiota = array_count_values($autod)["Toyota"];
$audi = array_count_values($autod)["Audi"];
echo "<br> Toitode arv on: " . $toiota . "<br> Audide arv on: " . $audi;
echo "<br><br>VIN koodid alla 17 tähemärki: <br>";

foreach ($nubrid as $uss){
  if(strlen($uss) < 17){
    echo $uss . "<br>";
  }
...
echo "<br>Keskmised palgad on: " . $keskmine . "<br>";
...
echo"<br>See riik on kõige pikima nimetusega :) ".$ajutineriik ." -Tähemärkide arv on: ". $maxriigid;
echo"<br>";
#Hiina names
...
echo"<br>Hiina nimed: ";
sort($hiinanimed);
foreach($hiinanimed as $hiina){
echo "<br> - ".$hiina;
}

echo"<br>Esimene nimi on ".$hiinanimed[0]." teine nimi on ".end($hiinanimed);
echo"<br><br>";
...
echo"<br> olemasolevad nimed on:  <br>";
foreach($google as $nimed) {
  echo $nimed.", ";
}
echo"<br>"
...
nimeotsing: <input type="text" name = "a" required><br>
<input type="submit" value = "otsing" class = "btn btn-success">
</form>
